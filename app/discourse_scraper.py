...
cookie_file_path = "cookies.txt" # Path to the cookie file

# get cookie string from file
def get_cookie_string(cookie_file_path):
    """Loads the raw cookie string from a file."""
    try:
        with open(cookie_file_path, "r", encoding="utf-8") as f:
            return f.read().strip()
    except Exception as e:
        print(f"Error reading cookie file '{cookie_file_path}': {e}")
        return ""

RAW_COOKIE_STRING = get_cookie_string("cookies.txt")
...
def save_topic_json(topic_id, json_data, output_dir):
    """Saves the topic JSON data to a file, including absolute thread and post URLs."""
    os.makedirs(output_dir, exist_ok=True)
    # Add absolute thread URL (with slug if available)
    topic_slug = json_data.get("slug") or json_data.get("topic_slug") or ""
    if topic_slug:
        thread_url = f"{DISCOURSE_BASE_URL}t/{topic_slug}/{topic_id}"
    else:
        thread_url = f"{DISCOURSE_BASE_URL}t/{topic_id}"
    json_data["thread_url"] = thread_url

    # Add absolute post URLs to each post
    posts = json_data.get("post_stream", {}).get("posts", [])
    for post in posts:
        # Use the relative post_url if present, else construct from slug/id/post_number
        rel_post_url = post.get("post_url")
        post_number = post.get("post_number")
        if rel_post_url:
            # Ensure no double slash
            abs_post_url = DISCOURSE_BASE_URL.rstrip("/") + rel_post_url
        elif topic_slug and post_number is not None:
            abs_post_url = f"{DISCOURSE_BASE_URL}t/{topic_slug}/{topic_id}/{post_number}"
        elif post_number is not None:
            abs_post_url = f"{DISCOURSE_BASE_URL}t/{topic_id}/{post_number}"
        else:
            abs_post_url = ""
        post["absolute_post_url"] = abs_post_url
